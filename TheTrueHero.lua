...
-- executor uyumlu hale getirilmiş, AYARLAR DEĞİŞMEDİ
...
-- tekrar atamalar (orijinal korunuyor)
chr = char
Head = h
Root = root
Torso = t
...
plr.Chatted:Connect(function(word)
	if word:sub(1,4) == "play" then
		...
		music:Play()
		wait(0.3)
		chatFunc("Şuan oynuyor "..game:service'MarketplaceService':GetProductInfo(music.SoundId:match("%d+")).Name.. "", 200, BrickColor.new('White'), 14)
	end
...
fall2.Volume = 4
fall2:Play()
  chatFunc2("S-Seni C-Canavar...", 400,14)
    	hum.MaxHealth = "1"
  Humanoid.Health = "-1"
  	  for i = 0, 44, 0.1 do
  	swait()
...
music.SoundId = "rbxassetid://"..ORGID
music.TimePosition = 0
chatFunc2("Vücudum.. Bölünmeye başlıyor gibi hissediyorum", 200,14)
	coroutine.resume(coroutine.create(function()
...
chatFunc2("Nasıl Olursa.. Milyonlarca parçaya bölündüm.", 200,14)
for i = 0, 15, 0.1 do
  	swait()
  	local Alpha = .05
  		hum.MaxHealth = math.huge
...
chatFunc2("Ruhumun derinliklerinde..", 200,14)
    for i = 0, 28, 0.1 do
  	swait()
  	local Alpha = .05
  		hum.MaxHealth = math.huge
...
chatFunc2("Yanan bir his ÖLMEME İZİN VERMİYOR!!", 200,14)
for i = 0, 15, 0.1 do
  	swait()
  	local Alpha = .05
  		hum.MaxHealth = math.huge
...
chatFunc2("Bundan daha zorunu denemelisin!", 200,14)
for i = 0, 11, 0.1 do
swait()
SWL.C0 = clerp(SWL.C0, CF(0, 0, 0) * angles(Rad(0), Rad(0), Rad(0)), 0.1)
RH.C0=clerp(RH.C0,cf(1,-1,0)*angles(math.rad(0),math.rad(90),math.rad(0))*angles(math.rad(-6),math.rad(0),math.rad(-6)),.3)
