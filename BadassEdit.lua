...
local FACE = CreatePart(3, Weapon, "Neon", 0, 0, "Dark orange", "Face mask", VT(1.001,1.001,1.001),false)
CreateWeldOrSnapOrMotor("Weld", FACE, Head, FACE, CF(0.02,0,0) * ANGLES(RAD(0), RAD(0), RAD(0)), CF(0, 0, 0))
MakeForm(FACE,"Head")
local EYE = CreatePart(3, Weapon, "Neon", 0, 0, "Tr. Red", "Eye", VT(0.1,0.25,0.2),false)
...
bottom.Name = "Cloth"
local BATON = CreatePart(3, Weapon, "Neon", 0, 0, "Dark green", "Baton", VT(1, 0.8, 3),false)
...
local GUN = CreatePart(3, Weapon, "Neon", 0, 0, "Tr. Blue", "Gun", VT(1, 0.8, 3),false)
...
local ATTACKSGOODCOP = {"Mouse - Uyarı","Q - Baton Sersemletme","E - Kelepçeler"}
local ATTACKSBADCOP = {"Mouse - Sessizlik","Q - Baton Kırıcı","E - Sessizliğin Yasası"}
local GOODUI = {}
local BADUI = {}
...
local SKILLTEXT = CreateLabel(SKILLFRAME, "[F - Değişim]", SKILLTEXTCOLOR, SKILLTEXTSIZE, SKILLFONT, 0, 2, 0, "Skill text")
...
function ApplyDamage(Humanoid,Damage)
	...
	if DEAD == true then
		if MODE == "GoodCop" then
			local TEXTS = {"Sadece kanunu uyguluyorum.","Kanunu hesaba katmalıydın.","Bir suçlu daha etkisiz hale getirildi."}
			Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
		elseif MODE == "BadCop" then
			local TEXTS = {"Suçluların bu dünyada yeri yok!","Kanunu hesaba katmalıydın!","Orada kal!"}
			Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
		end
	end
end
	...
	if TARGET ~= nil then
		if TARGET.Parent:FindFirstChildOfClass("Humanoid") then
			...
			if ROOT and HUM.Health > 0 then
				...
				Chatter("Sessiz kalma hakkına sahipsin.",0)
				...
				ROOT.Anchored = false
				Chatter("Söylediğin her şey aleyhine kullanılabilir ve kullanılacaktır.",0)
...
function BatonBreaker()
	...
	Rooted = false
	Speed = 4
	Chatter("Seni paramparça edicem!",0)
	for i=0, 0.2, 0.1 / Animation_Speed do
		Swait()
		RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CF(0, 0, 0) * ANGLES(RAD(0), RAD(0), RAD(0)), 1 / Animation_Speed)
		Neck.C0 = Clerp(Neck.C0, NECKC0 * CF(0, 0, 0 + ((1) - 1)) * ANGLES(RAD(15), RAD(0), RAD(-15)), 1 / Animation_Speed)
				...
				if PASS == true then
					...
					coroutine.resume(coroutine.create(function()
						...
						if HEAD then
							...
							Debris:AddItem(bv,0.05)
							local TEXTS = {"Suçluların bu dünyada yeri yok!","Orada kal!"}
							Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
						end
...
function KeyDown(Key)
	...
	if Key == "t" and ATTACK == false then
		...
		elseif MODE == "BadCop" then
			Chatter("Buna katılmıyorum, suçlu!",0.01)
		end
	end
...
while true do
	...
	sick.SoundId = "rbxassetid://99521953914772"
	sick.Looped = true
	sick.Pitch = 1
	sick.Volume = 4
	sick.Playing = true
