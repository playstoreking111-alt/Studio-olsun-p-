...
local ATTACKSGOODCOP = {"Fare - Uyarı","Q - Toma Tokatı","E - Kelepçe"}
local ATTACKSBADCOP = {"Fare - Sessizlik","Q - Toma Kırıcı","E - Susma Hakkı"}
local GOODUI = {}
local BADUI = {}
...
function ApplyDamage(Humanoid,Damage)
    Damage = 0 -- Hiçbir hasar uygulanmasın
    local DEAD = false
    ...
    if DEAD == true then
        if MODE == "GoodCop" then
            local TEXTS = {"Ben sadece kanunu uyguluyorum.","Kanunu dikkate almalıydın.","Bir suçlu daha yere serildi."}
            Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
        elseif MODE == "BadCop" then
            local TEXTS = {"Suçlular bu dünyadan gitmeli!","Kanunu dikkate almalıydın!","Yerde kal!"}
            Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
        end
    end
end
				...
				if PASS == true then
					...
					coroutine.resume(coroutine.create(function()
						...
						if HEAD then
							...
							Debris:AddItem(bv,0.05)
							local TEXTS = {"Suçlular bu dünyadan gitmeli!","Yerde kalın!"}
							Chatter(TEXTS[MRANDOM(1,#TEXTS)],0)
						end
