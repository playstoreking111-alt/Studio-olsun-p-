...
script = game:GetObjects("rbxassetid://5570421940")[1]["_ZVN"]
...
local MusicID = 2320338952
local Pitch = 1
local VisSong = 3194855687;
...
local MobileGUI = script.ContextButtonFrame;

local MobileButton = script:FindFirstChild'ContextActionButton';
local ComboGUI = script:FindFirstChild'ComboGUI';

ComboGUI.Parent = PlrGui
MobileGUI.Parent = MobileSGUI

local ComboBG = ComboGUI.BG
local ComboLbl = ComboBG.Combo
local ComboMet = ComboBG.Meter
local ComboFill = ComboMet.Fill

pcall(function()Char.LeftWing:destroy()end)
pcall(function()Char.Halo:destroy()end)
local toggleTag = true
local txt = Instance.new("BillboardGui", FDless.character.Head)
txt.Adornee = Head
txt.Name = "NameDetect"
...
text.TextColor3 = Color3.new(0.2,0.34509803921,0.50980392156)
text.Text = "Galvanizing"

pcall(function() Char.ReaperShadowHead.Eye1.BrickColor = BrickColor.new'Maroon' end)
pcall(function() Char.ReaperShadowHead.Eye2.BrickColor = BrickColor.new'Maroon' end)
...
for _,v in next, Feather:children() do v.CanCollide = false v.BrickColor = BrickColor.new'Storm blue' v.Material = Enum.Material.Neon v.CustomPhysicalProperties = PhysicalProperties.new(0,0,0,0,0) end
...
local Core2 = New("Part",Core,"Core2",{BrickColor = BrickColor.new("Storm blue"),CanCollide=false,Material = Enum.Material.Neon,FormFactor = Enum.FormFactor.Symmetric,Size = Vector3.new(0.349999994, 0.349999994, 1.06999981),CFrame = CFrame.new(-11.8130217, 33.4548035, 27.3183327, -8.05594027e-08, 8.28877091e-08, 1.00000942, 0.707106829, 0.707106829, 9.31322575e-10, -0.707110167, 0.707110226, 2.70083547e-08),BackSurface = Enum.SurfaceType.SmoothNoOutlines,BottomSurface = Enum.SurfaceType.SmoothNoOutlines,FrontSurface = Enum.SurfaceType.SmoothNoOutlines,LeftSurface = Enum.SurfaceType.SmoothNoOutlines,RightSurface = Enum.SurfaceType.SmoothNoOutlines,TopSurface = Enum.SurfaceType.SmoothNoOutlines,})
...
for i = 1, 360,5 do
	local part = NewInstance("Part",Halo,{BrickColor=BrickColor.new"Storm blue",Material=Enum.Material.Neon,Size=V3.N(0.25,0.1,0.1),Anchored=false,CanCollide=false,Locked=true})
	local weld = NewInstance("Weld",part,{Part0=HaloHandle,Part1=part,C0=CF.A(0,M.R(i),0)*CF.N(0,0,-1)})
	table.insert(PrimaryParts,part)
end

for i = 1, 6 do
	local cr = Part(Char,BrickColor.new'Storm blue',Enum.Material.SmoothPlastic,V3.N(.05,.05,.05),CF.N(),false,false)
	cr.Transparency = 1
...
function SongSmash()
	...
	local pl = Music.PlaybackLoudness
...
function MurderMode()
	...
	for _,v in next, LWing:children() do
		if(v:IsA'BasePart' and v.Name ~= 'Main')then v.Material = Enum.Material.Neon v.Transparency = 0 v.BrickColor = BrickColor.new'Storm blue' end
	end
	for _,v in next, RWing:children() do
		if(v:IsA'BasePart' and v.Name ~= 'Main')then v.Material = Enum.Material.Neon v.Transparency = 1 v.BrickColor = BrickColor.new'Storm blue' end
	end
	...
	for _,v in next, Halo:children() do
		if(v:IsA'BasePart' and v.Name ~= 'Main')then v.Material = Enum.Material.Neon v.BrickColor = BrickColor.new'Storm blue' end
	end
	...
	WalkSpeed = 16
	Mode = 1
	Core2.BrickColor = BrickColor.new'Storm blue'
	...
	RecolorTextAndRename("Galvanizing",Color3.new(0.2,0.34509803921,0.50980392156),Color3.new(0,0,0))
...
function PureMode()
	...
	RecolorTextAndRename("Sidestep",C3.N(1,1,1),BrickColor.new'Pastel Blue'.Color)
	MusicID = 888702733
	Pitch = 1
	Mode = 2
...
function VisMode()
	...
	pcall(function() Char.ReaperShadowHead.Eye2.Color = C3.N(1,1,1) end)
	WalkSpeed = 16
	RecolorTextAndRename("Rhythm",C3.N(1,1,1),C3.N(0,0,0))
	MusicID = VisSong
	Pitch = 1
	Mode = 6
	Core2.BrickColor = BrickColor.new'White'
...
Mouse.KeyDown:connect(function(code)
	if(code == "1" and Mode ~= 1)then
		MurderMode()
	elseif(code == "2" and Mode ~= 2)then
		PureMode()
	elseif(code == "3" and Mode ~= 3)then
		GlitchMode()
	elseif(code == "4" and Mode ~= 4)then
		StressMode()
	elseif(code == "5" and Mode ~= 5)then
		JusticeMode()
	elseif(code == "6" and Mode ~= 6)then
		VisMode()
	elseif(code == "7" and Mode ~= 7)then
		IceMode()
	elseif(code == "8" and Mode ~= 8)then
		LoveMode()
	elseif(code == "9" and Mode ~= 9)then
		ToxicMode()
	elseif(code == "0" and Mode ~= 10)then
		FluctuateMode()
	-- Transformations
	elseif(code == "t")then
		if(Mode == 1)then
...
if(UIS.TouchEnabled)then
	...
	table.insert(ModeButtons,BuildModeButton("Galvanizing",Vector2.new(500,205),MurderMode,1))
	table.insert(ModeButtons,BuildModeButton("Sidestep",Vector2.new(500,155),PureMode,2))
