...
local MODULE = script
local REMOTE = MODULE:FindFirstChild("Event")
...
local BulletStuff = {
    BulletSpeed = 0.05,
    AttachedTo = "Arm" -- Merminin kola mı yoksa motora mı bağlı olacağını seçer
}
local ANIM_SPEED = 3
...
BulletWeld.Part0 = CHARACTER["Right Arm"]
...
MOUSE.Button1Down:Connect(function()
	...
	repeat
		...
		makeafuckingtween(CHARACTER["Right Arm"].Position)
		task.wait()
...
MOUSE.Button1Up:Connect(function()
	...
	repeat task.wait()
		if BulletStuff.AttachedTo == "Motor" then
			makeafuckingtween(CHARACTER["Motor"].Position)
