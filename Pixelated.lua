--Made by Switcher (MoonOfSkys)

--[[ THE ABSOLUTE GOD ]]--
    --[[PIXELATION ‎▣]]--
 --[[ 1 in 1073741824 ]]--

-- EVEN GOT SPECIAL SONG
...
A script made by Switcher
Inspired by Sol's RNG

Any other copies of this script
will NOT be updated manually

Anyone claiming they made it
other than Switcher is lying
]]
...
if not usingHYPERGONER then
		task.spawn(function() 
			...
			if key == Enum.KeyCode.N or key == "n" then
				if not SOUL_KILL then
					SOUL_KILL = true
					chatFunc4("PIXELATED mode Enabled!")
				else
					SOUL_KILL = false
					chatFunc4("PIXELATED mode Disabled!")
				end
			end
		...
		if key == Enum.KeyCode.Six or key == "6" and mode ~= 7 then
			...
			chatFunc2("Now, ENOUGH messing around!")
				task.wait(3)
				chatFunc2("It's Time to Make Everyone PIXELATED")
				task.wait(4)
				if Head:FindFirstChild("NameBillboard") then
					Head:FindFirstChild("NameBillboard"):Destroy()
				end
