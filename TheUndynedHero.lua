...
-- EXECUTOR UYUMLU / İŞLEV DEĞİŞMEDEN DÜZELTİLMİŞ
...
cam.Focus = char:WaitForChild("Head").CFrame
...
RLeg = rl
LLeg = ll
RJ = root:WaitForChild("RootJoint")
Neck = tors:WaitForChild("Neck")
LS = tors:WaitForChild("Left Shoulder")
LH = tors:WaitForChild("Left Hip")
RS = tors:WaitForChild("Right Shoulder")
RH = tors:WaitForChild("Right Hip")
...
-- BodyGyro
local Directer = Inst("BodyGyro")
Directer.Parent = root
...
-------------------------------------------------------
-- Anti NoSounds (executor uyumlu)
-------------------------------------------------------
spawn(function()
	while task.wait() do
		if root and not root:FindFirstChild("GlitchS") then
...
scrg.Name = "Movers"
scrg.Parent = plr:WaitForChild("PlayerGui")

local massv = Instance.new("ImageLabel")
massv.Parent = scrg
...
-- PART
function parts(parent,name,size,color,mat,ref,tra)
...
-- WEDGE
function cwedges(parent,name,size,color,mat,ref,tra)
	local wedge = Create("CornerWedgePart"){
		Parent = parent,
		Name = name,
		Size = size,
		CanCollide = false,
...
-- MESH
function meshs(parent,name,scale,mtype,id)
...
-- SOUND
function sounds(parent,id,vol,pit,emitter)
...
-- SHADOW HEAD
local shadowhead = Create("Part"){
	Parent = chr.Head,
	BrickColor = BrickColor.new("Really black"),
	Size = Vector3.new(1.2, 0.6, 1),
	CanCollide = false,
	Anchored = false
}
local shadowheadmesh = Create("SpecialMesh"){
	Parent = shadowhead,
	MeshType = Enum.MeshType.Head,
	Scale = Vector3.new(1.26,1.5,1.26)
}
local weld2 = Create("Weld"){
	Parent = shadowhead,
	Part0 = shadowhead,
	Part1 = chr.Head,
	C1 = cf(0,0.2,0)
...
if leftArm and leftArm:IsA("BasePart") and leftArm.Size.Y == 2 then
...
local Players = game:GetService("Players")
local plr = owner or Players.LocalPlayer  -- owner tanımlı değilse LocalPlayer al
local h = h -- h'nin tanımlı olduğunu varsayıyoruz

local c = Instance.new("ObjectValue")
c.Name = "creator"
c.Value = Players:FindFirstChild(plr.Name) -- güvenli erişim
c.Parent = h 
...
local c = Instance.new("ObjectValue")
c.Name = "creator"
c.Value = game:GetService("Players"):FindFirstChild(owner and owner.Name or "") or game.Players.LocalPlayer
c.Parent = h
