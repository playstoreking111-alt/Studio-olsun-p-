...
writefile("Goner.mp3", game:HttpGet("https://github.com/playstoreking111-alt/Studio-olsun-p-/raw/refs/heads/main/G.O.N.E.R.%20%5BCRIMSON%20CHRONICLES%20OST%20GONER%20CLEETUS%20SKIN%20CHASE%20THEME%5D%20!!NOT%20FORSAKEN%20NOR%20PC2!!%20-%20Nell%20(youtube)%20(1).mp3"))
...
do
	...
	game:GetService("RunService").Heartbeat:Connect(function(s,p)
		...
		if TimeFrame >= Frame then
			...
			LastFrame=tick()
			TimeFrame -= Frame*math.floor(TimeFrame/Frame)
		end
	end)
	function swait(dur)
	...
	UserInputService.InputBegan:Connect(function(io)
		for n,t in pairs(CoAS.Actions) do
			for _,k in pairs(t.Keys) do
				if k==io.KeyCode then
					t.Function(t.Name,Enum.UserInputState.Begin,io)
				end
			end
		end
		...
		m:TrigEvent("KeyDown",io.KeyCode.Name)
	...
	UserInputService.InputEnded:Connect(function(io)
		for n,t in pairs(CoAS.Actions) do
			for _,k in pairs(t.Keys) do
				if k==io.KeyCode then
					t.Function(t.Name,Enum.UserInputState.End,io)
				end
			end
		end
		if io.UserInputType==Enum.UserInputType.MouseButton1 then
		...
		m:TrigEvent("KeyUp",io.KeyCode.Name)
	...
	local FakeService_Metatable = {
		__index=function(self,k)
			...
			if s then
				return typeof(s[k])=="function" and function(_,...)return s[k](s,...)end or s[k]
			end
		end,
...
local oldBreak = TARGET.BreakJoints
TARGET.BreakJoints = function() end
TARGET:BreakJoints()
TARGET.BreakJoints = oldBreak
...
if PRIMARY then
	local Spill = function(PART)
		if PART then
			task.spawn(function() -- coroutine yerine daha stabil
				local count = 0
				repeat
					Swait(MRANDOM(5,12))
					...
					count += 1
				until count >= 10 -- sonsuzluğu kestik
			end)
		end
	end
...
-- TARGET.Parent = Effects -- BUNU SİL, ana sorun bu
...
if TARGET:FindFirstChild("UpperTorso") then
	...
	if torso then
		local FOUNTAIN = IT("Attachment", torso)
		FOUNTAIN.Name = "LostNeck"
		FOUNTAIN.Position = VT(0, torso.Size.Y/2, 0)
	...
	if torso then
		...
		task.spawn(function()
	if ROOT:FindFirstChild("Kill") and ROOT:FindFirstChild("Taunt") then
		while ROOT.Kill and ROOT.Kill.Playing do
			task.wait(0.1)
		end
		if ROOT:FindFirstChild("Taunt") then
			ROOT.Taunt:Play()
		end
	end
end)
